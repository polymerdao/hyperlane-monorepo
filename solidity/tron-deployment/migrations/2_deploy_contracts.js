...
module.exports = async function (deployer, network, account) {
  ...
  const hyperlaneDomainId = process.env.HYPERLANE_DOMAIN_ID ? parseInt(process.env.HYPERLANE_DOMAIN_ID) : 728126428; // Default Tron Mainnet Domain ID
  ...
  const originChainMailboxAddress = process.env.ORIGIN_CHAIN_MAILBOX_ADDRESS || null;
  ...
  try {
    ...
    console.log('Deploying Mailbox implementation with domain ID:', hyperlaneDomainId);
    await deployer.deploy(Mailbox, hyperlaneDomainId);
    const mailboxImplementation = await Mailbox.deployed();
    console.log('✅ Mailbox implementation deployed at:', mailboxImplementation.address);
    ...
    console.log('Deploying proxy without initialization (will initialize separately)...');

    // --- Step 4: Deploy ERC1967Proxy ---
    console.log('\n🚀 Step 4: Deploying ERC1967Proxy for Mailbox...');
    await deployer.deploy(ERC1967Proxy, mailboxImplementation.address, encodedInitData);
    ...
    try {
      ...
      console.log('⚠️  Initialize failed (might already be initialized):', error.message);
    }
    ...
    try {
      const domain = await mailboxInstance.localDomain();
      console.log('✅ Mailbox local domain:', domain.toString());
      
      const currentOwner = await mailboxInstance.owner();
      console.log('✅ Mailbox owner:', currentOwner);
      
      const defaultIsm = await mailboxInstance.defaultIsm();
      console.log('✅ Default ISM:', defaultIsm);
      
      const defaultHook = await mailboxInstance.defaultHook();
      console.log('✅ Default Hook:', defaultHook);
      
      const requiredHook = await mailboxInstance.requiredHook();
      console.log('✅ Required Hook:', requiredHook);
    } catch (error) {
      console.log('⚠️  Verification failed (this might be expected):', error.message);
    }
    ...
    if (polymerProverAddress && originChainMailboxAddress) {
      ...
      console.log('  To deploy PolymerISM for receiving messages from other chains, provide:');
    ...
    console.log('  • Mailbox Implementation Address:', mailboxImplementation.address);
    ...
    console.log('  • As the `originMailbox` when deploying PolymerISM on the destination chain.');
    if (polymerIsm) {
      console.log('\n📨 Receiving Messages:');
      console.log('  • PolymerISM is deployed to verify messages from:', originChainMailboxAddress);
    ...
    console.log('MAILBOX_IMPLEMENTATION_ADDRESS=' + mailboxImplementation.address);
